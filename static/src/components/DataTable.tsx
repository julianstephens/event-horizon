import { getEvents } from "@/hooks/queries";
import type { Event } from "@/lib/api/aliases";
import { useEffect, useState } from "react";

export const DataTable = () => {
  const [cols, setCols] = useState<string[]>([]);
  const [rows, setRows] = useState<Event[]>([]);
  const { data, isSuccess, error } = getEvents();
  ...
  useEffect(() => {
    if (!isSuccess) {
      ...
      console.log(data);
      setCols(Object.keys(data?.data[0] as object));
      setRows(data?.data as Event[]);
    }
  }, [isSuccess]);
